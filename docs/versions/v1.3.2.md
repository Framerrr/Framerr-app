...
- **Template Sharing (Admin)** - User copy model:
  ...
  - **Sensitive widget config is stripped** from shared copies (API keys, links, etc.)
  - **Consolidated sharing logic** - `shareTemplateWithUser()` helper handles all sharing flows
- **Template Preview Expand Animation** - iOS-style shared element transition when opening template preview
  - Thumbnail smoothly scales/morphs into full preview modal
  - Uses Framer Motion `layoutId` for seamless animation
...
- **Fixed template preview modal on mobile** - now uses createPortal to render outside scrollable containers
  - Properly fixed to screen with correct max-height accounting for tab bar
  - Body scroll locked when modal open
- **Fixed flatten/showHeader settings not updating live** - widget config changes now apply immediately
  - Fixed event name mismatch (`widget-config-updated` → `widget-config-changed`)
  - Dashboard now processes config changes outside edit mode
...
### Settings Page Loading Spinners Standardized
- Replaced inconsistent loading indicators with standardized `LoadingSpinner` component across 10 settings pages
- Replaced RefreshCw arrow spinner in Dashboard → General
- Replaced text-only loading in Widget Gallery, Users, Tab Groups, My Tabs
- Replaced inline div spinners and Lucide icons in Profile, Notifications, Auth, System, Integrations
...
- Single source of truth for widget → integration mapping
