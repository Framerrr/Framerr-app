...
const Dashboard = () => {
    ...
    useEffect(() => {
        const handleWidgetConfigUpdate = async (event) => {
            ...
            if (!widgetId) return;
            logger.info('Widget config updated, refreshing single widget:', widgetId);
            try {
                ...
                logger.info('Widget updated successfully:', widgetId);
    ...
    useEffect(() => {
        const handleWidgetsModified = (event) => {
            ...
            logger.info('Widgets modified from LinkGrid, updating state');
    ...
    useEffect(() => {
        ...
        const cols = currentBreakpoint === 'xxs' || currentBreakpoint === 'xs' ? 2 : 12; // xs/xxs=2 (full width), md/sm=12
